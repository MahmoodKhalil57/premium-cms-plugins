...
import type { RouteContext, StorageCollection } from "../shim.js";
...
import { priceDeltas } from "../fields.js";
...
export async function submitHandler(ctx: RouteContext<SubmitInput>) {
	...
	const pricing = priceDeltas(allFields, result.data);
	const submission: Submission = {
		formId,
		data: result.data,
		...(pricing.total > 0 ? { total: pricing.total } : {}),
		...
		meta: {
			...
			referer: ctx.requestMeta?.referer ?? null,
