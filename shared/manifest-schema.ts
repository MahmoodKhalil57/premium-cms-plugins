...
// Vendored from premium-cms-image packages/core/src/plugins/manifest-schema.ts — keep in sync when the image changes the manifest format.
...
/** Plugin-contributed grant keys: `<pluginId>:<name>` (any depth). */
const GRANT_KEY_RE = /^[a-z0-9_-]+(:[a-z0-9_-]+)+$/;

const manifestGrantSchema = z.object({
	/** Short name; the runtime prefixes it with the plugin id. */
	key: z.string().min(1).max(64).regex(/^[a-z0-9_-]+(:[a-z0-9_-]+)*$/, "Invalid grant key"),
...
const pluginMcpConfigSchema = z.object({
	tools: z.array(
		z.object({
			...
			permission: z.string().refine((permission) => permission in Permissions || GRANT_KEY_RE.test(permission), "Unknown permission"),
...
const baseSettingFields = {
	...
	/** Only show (and only require) this field when another setting has the given value. */
	showWhen: z.object({ field: z.string().max(64), equals: z.union([z.string(), z.boolean()]) }).optional(),
};
...
const settingFieldSchema = z.discriminatedUnion("type", [
	...
	// Read-only note, e.g. a webhook URL the operator copies into a provider. `{{origin}}` expands to the site origin.
	z.object({ ...baseSettingFields, type: z.literal("info"), value: z.string().max(2000).optional() }),
	z.object({
		...baseSettingFields,
		type: z.literal("url"),
		default: z.string().optional(),
...
export const pluginManifestSchema = z.object({
	...
	/** Grants this plugin contributes; roles can be given them. */
	grants: z.array(manifestGrantSchema).max(100).optional(),
});
